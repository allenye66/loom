...
import { useTasks, useTaskActions } from '../api';
...
export function TasksView({ repoRoot }: { repoRoot: string }) {
  ...
  // Create the worktree task, then drop straight into a fresh chat in it.
  const submit = async () => {
    ...
    try {
      ...
      if (task?.worktree_path) openChat({ cwd: task.worktree_path, resume: task.chat_id ?? undefined, title: task.branch });
    } catch {
